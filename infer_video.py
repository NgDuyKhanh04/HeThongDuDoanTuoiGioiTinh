...
def ema(o,n,a=ALPHA): return n if o is None else o*(1-a)+n*a
...
def main():
    if len(sys.argv)<2: print("python infer_video.py <file_video>"); return
    path=sys.argv[1]
    if not os.path.exists(path): print("❌ Không thấy file:",path); return

    model=load_model(MODEL, compile=False)
    cap=cv2.VideoCapture(path)
    if not cap.isOpened(): print("❌ Không mở được video"); return

    out_path=os.path.splitext(path)[0]+"_out.mp4"
    ...
    while True:
        ...
        for i,(x,y,fw,fh) in enumerate(faces):
            ...
            draw(frame,x,y,fw,fh,f"{gender}, {scope}")

        tracks=new_tracks
        out.write(frame)
        cv2.imshow("Video Age+Gender (v3)", frame)
